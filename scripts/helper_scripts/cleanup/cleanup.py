...
class CleanDeployment:
    ...
    def collect_cr_details(self):
        ...
        self._cr_name = cr_details.get("name", 'content')
        version = cr_details.get("version", "26.0.0")
    ...
    def delete_CR(self, task1, progress, cr_type="Content", group="fncm.ibm.com",
                  version="v1", plural="fncmclusters", cr_name=None):
        """
        Delete a Custom Resource (CR) and its associated resources.
        
        Args:
            task1: Progress task ID
            progress: Progress instance for logging
            cr_type: Type of CR ("Content" or "AI Services")
            group: API group (e.g., "fncm.ibm.com" or "ccxaiservices.operator.ibm.com")
            version: API version (e.g., "v1")
            plural: Resource plural (e.g., "fncmclusters" or "ccxaiservices")
            cr_name: Name of the CR (defaults to self._cr_name)
        """
        SLEEP_TIMER = 5
        cr_name = cr_name or self._cr_name
        
        # Attempt to list the CR in the specific namespace
        progress.log(f"Starting IBM Content Cortex {cr_type} Deployment Cleanup")
        progress.log()
        self._logger.info(f"Starting IBM Content Cortex {cr_type} Deployment Cleanup")
        ...
        self._logger.info(f"Deleting {cr_type} CR")
    ...
    def delete_operator(self, task1, progress):
        ...
        if self._deployment_prerequisites.platform.lower() in ["ocp"]:
            ...
            progress.log("Starting IBM Content Cortex Operator OLM Uninstallation")
            progress.log()
            ...
            if provided_apis and len(provided_apis) > 1:
                ...
                progress.log()
                progress.log("Updating Operator Group to remove the Operator API served by IBM Content Cortex Operator")
                self._logger.info(f"Updating Operator Group to remove the Operator API served by IBM Content Cortex Operator")
            ...
            progress.log()
            progress.log(Panel.fit("Uninstalling IBM Content Cortex Operator completed!", style="bold green"))
            progress.log()
            ...
            progress.log("Starting IBM Content Cortex Operator Yaml Uninstallation")
            progress.log()
