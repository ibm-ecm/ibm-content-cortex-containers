...
class Validate:
    ...
    def __init__(self, logger,
        ...
        # Only initialize database-related paths if db_prop is provided
        if self._db_prop and "DATABASE_TYPE" in self._db_prop:
            ...
            # Set to None when no database configuration is provided
            self._DB_JDBC_PATH = None
            self._DB_CONNECTION_JAR_PATH = None
    ...
    def validate_all_db(self, task3, progress):
        ...
        if db_type == "postgresql":
            max_transactions = Panel.fit(Text(
                ...
                "Please see https://www.ibm.com/docs/SSL4SY_26.0.0/com.ibm.p8.performance.doc/p8ppi308.htm.",
                style="bold green"))
        ...
        if db_type == "sqlserver":
            xa_enabled = Panel.fit(Text(
                ...
                "Please see https://www.ibm.com/docs/SSL4SY_26.0.0/com.ibm.p8.planprepare.doc/p8ppi027.htm.",
                style="bold green"))
    ...
    def create_truststore(self, progress):
        ...
        dname = self._dnsname
        alias = self._alias
        # Use a default password if KEYSTORE_PASSWORD is not available (AI services only scenario)
        truststore_pwd = self._user_group_prop.get("KEYSTORE_PASSWORD", "changeit")
    ...
    def get_admin_users_scim(self):
        ...
        if "FNCM_LOGIN_USER" in self._user_group_prop.keys():
            ...
            users_dict["IBM Content Cortex Admin"] = user
    ...
    def get_unique_storageclass(self) -> set:
        # Return empty set if deployment properties are not available
        if not self._deploy_prop:
            return set()
        
        sc_set = {
            self._deploy_prop.get("SLOW_FILE_STORAGE_CLASSNAME"),
            self._deploy_prop.get("MEDIUM_FILE_STORAGE_CLASSNAME"),
            self._deploy_prop.get("FAST_FILE_STORAGE_CLASSNAME")
        }
    ...
    def auto_apply_all_secrets_in_folder(self, folder_path):
        ...
        with Progress(
            ...
            console=self._logger.handlers[0].console if hasattr(self._logger.handlers[0], 'console') else None
        ) as progress:
            task = progress.add_task(f"[cyan]Applying secrets from {os.path.basename(folder_path)}...", total=len(files))
            ...
            for f in files:
                ...
                if applied:
                    ...
                    print(f"  [green]✓[/green] Secret Applied: [cyan]{f}[/cyan]")
                else:
                    failed_files.append(f)
                    print(f"  [red]✗[/red] Failed to apply secret: [red]{f}[/red]")
    ...
    def _apply_vault_secrets_in_folder(self, folder_path):
        """
        Apply only SecretProviderClass resources from a folder when Vault is enabled.
        Skips regular Kubernetes Secret resources and JSON vault data files.
        """
        from rich.progress import Progress, SpinnerColumn, TextColumn, BarColumn
        import yaml
        ...
        with Progress(
            ...
            task = progress.add_task(f"[cyan]Applying Vault SecretProviderClass from {os.path.basename(folder_path)}...", total=len(files))

            for f in files:
                file_path = os.path.join(folder_path, f)
                
                # Read the file to check if it's a SecretProviderClass
                try:
                    with open(file_path, 'r') as file:
                        content = yaml.safe_load(file)
                        
                    # Check if this is a SecretProviderClass resource
                    if content and isinstance(content, dict):
                        kind = content.get('kind', '')
                        
                        if kind == 'SecretProviderClass':
                            # Apply SecretProviderClass
                            self._logger.info(f"Applying SecretProviderClass from file: {f}")
                            applied = self._kube.apply_cluster_resource_files("secretproviderclass", file_path, namespace=self._namespace)
                            
                            if applied:
                                applied_files.append(f)
                                print(f"  [green]✓[/green] SecretProviderClass Applied: [cyan]{f}[/cyan]")
                            else:
                                failed_files.append(f)
                                print(f"  [red]✗[/red] Failed to apply SecretProviderClass: [red]{f}[/red]")
                        else:
                            # Skip non-SecretProviderClass resources (regular Secrets)
                            skipped_files.append(f)
                            self._logger.info(f"Skipping non-SecretProviderClass resource: {f} (kind: {kind})")
                            print(f"  [dim]⊘[/dim] Skipped (not SecretProviderClass): [dim]{f}[/dim]")
                    else:
                        skipped_files.append(f)
                        self._logger.warning(f"Could not parse YAML file: {f}")
                        print(f"  [yellow]⚠[/yellow] Skipped (invalid YAML): [yellow]{f}[/yellow]")
                        
                except Exception as e:
                    failed_files.append(f)
                    self._logger.error(f"Error processing file {f}: {str(e)}")
                    print(f"  [red]✗[/red] Error processing: [red]{f}[/red] - {str(e)}")

                progress.advance(task)

        # Summary
        if applied_files or failed_files or skipped_files:
            print()
    ...
    def auto_apply_secrets_ssl(self):
        ...
        vault_enabled = self._deploy_prop.get("VAULT_ENABLED", False)

        # Display header with vault status
        header = Text()
        header.append("📦 ", style="bold cyan")
        if vault_enabled:
            header.append("Applying Vault SecretProviderClass Resources", style="bold white")
        else:
            header.append("Applying SSL Certificates and Secrets", style="bold white")
        ...
        if vault_enabled:
            ...
            vault_spc_folder = os.path.join(generated_folder, "vault", "secret-provider-classes")
            if os.path.exists(vault_spc_folder):
                self._logger.info(f"Applying SecretProviderClass resources from: {vault_spc_folder}")
                self._apply_vault_secrets_in_folder(folder_path=vault_spc_folder)
            else:
    ...
    def auto_apply_configmaps(self):
        """Apply all ConfigMaps from the configmaps folder."""
        from rich.console import Console
        from rich.panel import Panel
        from rich.text import Text
        ...
        console = Console()
        generated_folder = os.path.join(os.getcwd(), "generatedFiles", self._namespace)
        configmaps_folder = os.path.join(generated_folder, "configmaps")

        if not os.path.exists(configmaps_folder):
            self._logger.info(f"No configmaps folder found at {configmaps_folder}")
            return

        # Display header
        header = Text()
        header.append("⚙️ ", style="bold cyan")
        header.append("Applying ConfigMaps", style="bold white")
        console.print(Panel(header, border_style="cyan", padding=(0, 2)))
        print()

        yaml_ext = [".yaml", ".yml"]
        files = self.__files_in_dir(configmaps_folder, yaml_ext)

        if len(files) == 0:
            self._logger.info(f"No ConfigMap files found in {configmaps_folder}")
            print()
            return

        for f in files:
            self._logger.info(f"Applying ConfigMap from file: {f}")
            applied = self._kube.apply_cluster_resource_files("configmap", os.path.join(configmaps_folder, f), namespace=self._namespace)

            if applied:
                print(f"  [green]✓[/green] ConfigMap Applied: [cyan]{f}[/cyan]")
            else:
                print(f"  [red]✗[/red] Failed to apply ConfigMap: [red]{f}[/red]")

        print()

    def auto_apply_metrics(self):
        """Apply all metrics resources (IBMServiceMeterDefinition) from the metrics folder."""
        from rich.console import Console
        from rich.panel import Panel
        from rich.text import Text
        ...
        console = Console()
        generated_folder = os.path.join(os.getcwd(), "generatedFiles", self._namespace)
        metrics_folder = os.path.join(generated_folder, "metrics")

        if not os.path.exists(metrics_folder):
            self._logger.info(f"No metrics folder found at {metrics_folder}")
            return

        # Display header
        header = Text()
        header.append("📊 ", style="bold cyan")
        header.append("Applying Metrics (IBMServiceMeterDefinition)", style="bold white")
        console.print(Panel(header, border_style="cyan", padding=(0, 2)))
        print()

        # Add info message about operator requirement
    ...
    def auto_apply_cr(self):
        ...
        header = Text()
        header.append("🚀 ", style="bold cyan")
        header.append("Applying Custom Resources", style="bold white")
        console.print(Panel(header, border_style="cyan", padding=(0, 2)))
        print()

        success = True

        # Apply Content CR (main FNCM CR)
        content_cr_file = os.path.join(generated_folder, "ibm_content_cr_production.yaml")
        if os.path.exists(content_cr_file):
            self._logger.info(f"Applying Content CR: ibm_content_cr_production.yaml")
            applied = self._kube.apply_cluster_resource_files("custom resource", content_cr_file, namespace=self._namespace)

            if applied:
                print(f"  [green]✓[/green] Content CR Applied: [cyan]ibm_content_cr_production.yaml[/cyan]")
            else:
                print(f"  [red]✗[/red] Failed to apply Content CR: [red]ibm_content_cr_production.yaml[/red]")
                success = False
        else:
            # Content CR is optional (may not exist for AI services-only deployments)
            self._logger.info(f"Content CR file not found (optional): {content_cr_file}")

        # Apply AI Services CR if it exists
        ai_services_cr_file = os.path.join(generated_folder, "ibm_ai_services_cr_production.yaml")
        if os.path.exists(ai_services_cr_file):
            self._logger.info(f"Applying AI Services CR: ibm_ai_services_cr_production.yaml")
            applied = self._kube.apply_cluster_resource_files("custom resource", ai_services_cr_file, namespace=self._namespace)

            if applied:
                print(f"  [green]✓[/green] AI Services CR Applied: [cyan]ibm_ai_services_cr_production.yaml[/cyan]")
            else:
                print(f"  [red]✗[/red] Failed to apply AI Services CR: [red]ibm_ai_services_cr_production.yaml[/red]")
                success = False
        else:
    ...
    # ============================================================================
    # ValidationDisplay Wrapper Methods
    # ============================================================================
    # These methods wrap existing validation methods to work with ValidationDisplay

    def validate_all_storage_classes_with_display(self, display):
        """Wrapper for validate_all_storage_classes that works with ValidationDisplay"""
        ...
        for storage_class in sc_set:
            display.add_detail(f"Validating storage class: {storage_class}")

            # Create adapter for this validation
            adapter = DisplayProgressAdapter(display, "storage")

            # Call existing validation method
            self.validate_sample_sc(storage_class, "ReadWriteMany", "fncm-test-pvc", 0, adapter)

            completed += 1
            display.update_test("storage", completed, f"Validated {completed}/{total} storage classes")

        # Check if all storage classes passed
        all_passed = all(self.is_validated.get(sc, False) for sc in sc_set)
        return all_passed

    def validate_all_db_with_display(self, display):
        """Wrapper for validate_all_db that works with ValidationDisplay"""
        from .validation_display import DisplayProgressAdapter

        # Create adapter for this validation
        adapter = DisplayProgressAdapter(display, "database")

        # Call existing validation method
        self.validate_all_db(0, adapter)

        # Check if all databases passed - need to check GCD, Object Stores, and ICN
        databases_to_check = []
        
        # Add GCD if CPE is enabled
        if "CPE" in self._deploy_prop.keys() and self._deploy_prop["CPE"]:
            databases_to_check.append("GCD")
            # Add all Object Store databases
            databases_to_check.extend(self._db_prop.get("_os_ids", []))
        
        # Add ICN if BAN is enabled
        if "BAN" in self._deploy_prop.keys() and self._deploy_prop["BAN"]:
            databases_to_check.append("ICN")
        
        # Check validation status for all databases
        all_passed = all(self.is_validated.get(db_id, False) for db_id in databases_to_check)
        return all_passed

    def validate_all_ldap_with_display(self, display):
        """Wrapper for validate_all_ldap that works with ValidationDisplay"""
        from .validation_display import DisplayProgressAdapter

        # Create adapter for this validation
        adapter = DisplayProgressAdapter(display, "ldap")

        # Call existing validation method
        result = self.validate_all_ldap(0, adapter)

        return result

    def validate_ldap_users_groups_with_display(self, display):
        """Wrapper for validate_ldap_users_groups that works with ValidationDisplay"""
        from .validation_display import DisplayProgressAdapter

        # Create adapter for this validation
        adapter = DisplayProgressAdapter(display, "ldap_users")

        # Call existing validation method
        try:
            self.validate_ldap_users_groups(0, adapter)
            # This method doesn't set validation status, so if it completes without exception, it passed
            return True
        except Exception as e:
            display.add_detail(f"Error validating LDAP users/groups: {str(e)}")
            return False

    def validate_all_idps_with_display(self, display):
        """Wrapper for validate_all_idps that works with ValidationDisplay"""
        from .validation_display import DisplayProgressAdapter

        # Create adapter for this validation
        adapter = DisplayProgressAdapter(display, "idp")

        # Call existing validation method
        self.validate_all_idps(0, adapter)

        # Check if all IDPs passed
        idp_ids = self._idp_prop.get("_idp_ids", [])
        all_passed = all(self.is_validated.get(idp_id, False) for idp_id in idp_ids)
        return all_passed

    def validate_scim_with_display(self, display):
        """Wrapper for validate_scim that works with ValidationDisplay"""
        from .validation_display import DisplayProgressAdapter

        # Create adapter for this validation
        adapter = DisplayProgressAdapter(display, "scim")

        # Call existing validation method
        self.validate_scim(0, adapter)
