...
class GenerateCR:
    ...
    def __init__(self, db_properties={}, ldap_properties={}, usergroup_properties={}, deployment_properties={},
                 ingress_properties={}, customcomponent_properties={}, idp_properties={}, scim_properties={},
                 logger: Optional[Logger]=None, namespace=""):
        self._logger = logger
        ...
        cr_templates_base = Path.cwd() / "helper_scripts" / "generate" / "cr_templates" / self.ccx_version / "content" 
        self._base_template = cr_templates_base / "base.yaml"
        self._database_template = cr_templates_base / "database.yaml"
        self._ldap_template = cr_templates_base / "ldap.yaml"
        self._idp_template = cr_templates_base / "idp.yaml"
        self._ingress_template = cr_templates_base / "ingress.yaml"
        self._egress_template = cr_templates_base / "egress.yaml"
        self._init_template = cr_templates_base / "init.yaml"
        self._multi_ldap_template = cr_templates_base / "ldap-multi.yaml"
        self._verify_template = cr_templates_base / "verify.yaml"
        self._scim_template = cr_templates_base / "scim.yaml"

        self._generated_cr = self._generate_folder / "ibm_content_cr_production.yaml"
        self._merged_data = CommentedMap()
        if self._generated_cr.exists():
            self._generated_cr.unlink()
    ...
    def populate_ldap_section(self, ldap_dict, cr_key, key=""):
    ...
    def populate_idp_section(self):
        ...
        try:
            ...
            for idx, key in enumerate(self._idp_properties["_idp_ids"]):
                ...
                # Set audiences to the same value as CLIENT_ID
                idp_section["spec"]["shared_configuration"]["open_id_connect_providers"][idx]["audiences"] = \
                    self._idp_properties[key]["CLIENT_ID"]
    ...
    def generate_base_section(self):
        ...
        try:
            ...
            # removing sc_deployment_patterns:content if one of cpe, graphql,ban is not selected
            ...
            trusted_certs_path = self._generate_folder / "ssl" / "trusted-certs"
            if trusted_certs_path.exists():
                trusted_cert_secrets = collect_visible_files(str(trusted_certs_path))
            ...
            ssl_path = self._generate_folder / "ssl"
            if ssl_path.exists():
                ssl_cert_secrets = collect_visible_files(str(ssl_path))
            ...
            if self._deployment_properties.get("VAULT_ENABLED", False):
                if "sc_vault_configuration" not in base_dict["spec"]["shared_configuration"]:
                    base_dict["spec"]["shared_configuration"]["sc_vault_configuration"] = CommentedMap()
                base_dict["spec"]["shared_configuration"]["sc_vault_configuration"]["enable_external_secret_store"] = True
        ...
        try:
            ...
            if len(feature_dict["TM"]) != 0 or len(feature_dict["CSS"]) != 0:
                ...
                self._merged_data['spec'].yaml_set_comment_before_after_key("ecm_configuration",
                                                                            before='########################################################################\n########   IBM Content Cortex configuration      ########\n########################################################################',
                                                                            indent=2)
